...
typedef void LoadedCallback(Geometry geometry, List<Material> materials);
...
class Loader {
  ...
  _initMaterials(List materials, String texturePath) {
    var array = [];
  ...
  _needsTangents(List materials) {
    var il = materials.length;
  ...
  _create_texture(texturePath, sourceFile, repeat, offset, wrap, anisotropy) {
    var isCompressed = new RegExp(r"/\.dds$/i").hasMatch(sourceFile);
    ...
    if(repeat != null) {
      ...
      if (repeat[0] != 1) result.wrapS = REPEAT_WRAPPING;
      if (repeat[1] != 1) result.wrapT = REPEAT_WRAPPING;
    }
    ...
    if (wrap != null) {
      var wrapMap = {"repeat": REPEAT_WRAPPING,
                     "mirror": MIRRORED_REPEAT_WRAPPING};
  ...
  _createMaterial(m, String texturePath) {
    Material material;
    ...
    if (m.containsKey("blending")) { // TODO - && THREE[m.blending] != undefined) {
      ...
      var blendmodes = {
        "NoBlending":          NO_BLENDING,
        "NormalBlending":      NORMAL_BLENDING,
        "AdditiveBlending":    ADDITIVE_BLENDING,
        "SubtractiveBlending": SUBTRACTIVE_BLENDING,
        "MultiplyBlending":    MULTIPLY_BLENDING,
        "CustomBlending":      CUSTOM_BLENDING };
