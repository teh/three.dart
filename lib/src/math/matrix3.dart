...
class Matrix3 {
  ...
  dynamic operator*(dynamic arg) {
    ...
    if (arg is Vector3) return _mul_vector(arg);
    if (3 == arg.dimension) return _mul_matrix(arg);
  ...
  /*
   * Additions based on three.js matrix3
   */
  
  /// Constructs a rotation matrix, looking from [eye] towards [target] with defined [up] vector.
  Matrix3.lookAt(Vector3 eye, Vector3 target, Vector3 up) {
    ...
    storage[0] = x.x; storage[3] = y.x; storage[6] = z.x;
    storage[1] = x.y; storage[4] = y.y; storage[7] = z.y;
    storage[2] = x.z; storage[5] = y.z; storage[8] = z.z;
  }
  
  void multiplyDoubleArray(List<double> array) {
    for (var i = 0; i < array.length; i += 3) {
      var v = transform(new Vector3.array(array, i));
      array[i]   = v.x;
      array[i+1] = v.y;
      array[i+2] = v.z;
    } 
