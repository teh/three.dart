...
class Vector3 {
  ...
  Vector3(double x, double y, double z) : storage = new Float32List.fromList([x, y, z]);
  ...
  /// Copy of [other].
  Vector3.copy(Vector3 other) : storage = new Float32List.fromList([other.x, other.y, other.z]);
  ...
  /*
   * Additions based on three.js vector3
   */
  
  // Used in polyhedron geometry.
  int _index;
  Vector2 _uv;
  
  /// Splat [arg] into all lanes of the vector.
  Vector3.splat(double arg) : storage = new Float32List.fromList([arg, arg, arg]);
  
  /// Constructs a vector with components [1.0, 1.0, 1.0].
  Vector3.one() : storage = new Float32List.fromList([1.0, 1.0, 1.0]);
  
  /// Constructs a vector with components [-1.0, -1.0, -1.0].
  Vector3.minusOne() : storage = new Float32List.fromList([-1.0, -1.0, -1.0]);
  
  /// Constructs a vector with components [1.0, 0.0, 0.0].
  Vector3.unitX() : storage = new Float32List.fromList([1.0, 0.0, 0.0]);
  
  /// Constructs a vector with components [0.0, 1.0, 0.0].
  Vector3.unitY() : storage = new Float32List.fromList([0.0, 1.0, 0.0]);
  
  /// Constructs a vector with components [0.0, 0.0, 1.0].
  Vector3.unitZ() : storage = new Float32List.fromList([0.0, 0.0, 1.0]);
  
  /// Constructs a vector with components [0.0, 1.0, 0.0].
  Vector3.up() : storage = new Float32List.fromList([0.0, 1.0, 0.0]);
  
  /// Constructs a vector with components [0.0, -1.0, 0.0].
  Vector3.down() : storage = new Float32List.fromList([0.0, -1.0, 0.0]);
  
  /// Constructs a vector with components [1.0, 0.0, 0.0].
  Vector3.right() : storage = new Float32List.fromList([1.0, 0.0, 0.0]);
  
  /// Constructs a vector with components [-1.0, 0.0, 0.0].
  Vector3.left() : storage = new Float32List.fromList([-1.0, 0.0, 0.0]);
  
  /// Constructs a vector with components [0.0, 0.0, -1.0].
  Vector3.forward() : storage = new Float32List.fromList([0.0, 0.0, -1.0]);
  
  /// Constructs a vector with components [0.0, 0.0, 1.0].
  Vector3.backward() : storage = new Float32List.fromList([0.0, 0.0, 1.0]);
  
  /// Constructs a vector with components [double.INFINITY, double.INFINITY, double.INFINITY].
  Vector3.infinity() : storage = new Float32List.fromList([double.INFINITY, double.INFINITY, double.INFINITY]);
  
  /// Constructs a vector with components [-double.INFINITY, -double.INFINITY, -double.INFINITY].
  Vector3.minusInfinity() : storage = new Float32List.fromList([-double.INFINITY, -double.INFINITY, -double.INFINITY]);
  
  /// Constructs a vector with components [arg.x, arg.y, 0.0].
  Vector3.vector2(Vector2 arg) : storage = new Float32List.fromList([arg.x, arg.y, 0.0]);
  
  /// Computes Manhattan length of this vector.
  /// http://en.wikipedia.org/wiki/Taxicab_geometry
  double lengthManhattan() => storage[0].abs() + storage[1].abs() + storage[2].abs();

  /// Applies a Quaternion transform to this vector.
  Vector3 applyQuaternion(Quaternion q) {
    // calculate quat * vector
    var ix =  q.w * storage[0] + q.y * storage[2] - q.z * storage[1];
    var iy =  q.w * storage[1] + q.z * storage[0] - q.x * storage[2];
    var iz =  q.w * storage[2] + q.x * storage[1] - q.y * storage[0];
    var iw = -q.x * storage[0] - q.y * storage[1] - q.z * storage[2];
    // calculate result * inverse quat
    storage[0] = ix * q.w + iw * -q.x + iy * -q.z - iz * -q.y;
    storage[1] = iy * q.w + iw * -q.y + iz * -q.x - ix * -q.z;
    storage[2] = iz * q.w + iw * -q.z + ix * -q.y - iy * -q.x;
    return this;
  }
  
  /// Linear Interpolation between this vector and [other], where [t] is the percent along the line.
  Vector3 lerp(Vector3 other, double t) {
    storage[0] += (other.x - storage[0]) * t;
    storage[1] += (other.y - storage[1]) * t;
    storage[2] += (other.z - storage[2]) * t;
    return this;
  }
  
  /// Multiplies [this] by [m].
  Vector3 applyMatrix3(Matrix3 m) {
    storage[0] = m[0] * storage[0] + m[3] * storage[1] + m[6] * storage[2];
    storage[1] = m[1] * storage[0] + m[4] * storage[1] + m[7] * storage[2];
    storage[2] = m[2] * storage[0] + m[5] * storage[1] + m[8] * storage[2];
    return this;
  }

  /// Multiplies [this] by [m].
  Vector3 applyMatrix4(Matrix4 m) {
    storage[0] = m[0] * storage[0] + m[4] * storage[1] + m[8]  * storage[2] + m[12];
    storage[1] = m[1] * storage[0] + m[5] * storage[1] + m[9]  * storage[2] + m[13];
    storage[2] = m[2] * storage[0] + m[6] * storage[1] + m[10] * storage[2] + m[14];
    return this;
  }
  
  /// Transforms the direction of [this] by a matrix (a 3 x 3 subset of [m]) 
  /// and then normalizes the result.
  Vector3 transformDirection(Matrix4 m) {
    storage[0] = m[0] * storage[0] + m[4] * storage[1] + m[8]  * storage[2];
    storage[1] = m[1] * storage[0] + m[5] * storage[1] + m[9]  * storage[2];
    storage[2] = m[2] * storage[0] + m[6] * storage[1] + m[10] * storage[2];
    normalize();
    return this;
  }
  
  /// Returns whether [this] is a zero vector
  bool get isZero => 
      storage[0] == 0.0 &&
      storage[1] == 0.0 &&
      storage[2] == 0.0;
