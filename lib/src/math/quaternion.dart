...
class Quaternion {
  ...
  Quaternion.fromRotation(Matrix3 rotationMatrix) : storage = new Float32List(4) {
    ...
    if (trace > 0.0) {
      ...
      int i = rotationMatrix.storage[0] < rotationMatrix.storage[4] ? (rotationMatrix.storage[4] < rotationMatrix.storage[8] ? 2 : 1)
  ...
  Quaternion.fromFloat32List(this.storage);
  ...
  /*
   *  Additions based on three.js quaternion
   */
  ...
  void _updateEuler() {
    if (_euler != null) {
      _euler.setFromQuaternion(this);
    }
  }
  
  Quaternion.fromEuler(Euler euler, [bool update = false]) : storage = new Float32List(4) {
    setFromEuler(euler, update);
  }
  
  /// Rotation of [angle] around [axis].
  Quaternion.fromAxisAngle(Vector3 axis, double angle) : storage = new Float32List(4) {
    setFromAxisAngle(axis, angle);
  }

  
  /// Linearly interpolates between [this] and [q2] by [t].
  Quaternion lerp(Quaternion q2, double t) {
    Quaternion q = (((storage[0] * q2.x) + (storage[1] * q2.y)) + (storage[2] * q2.z)) + (storage[3] * q2.w) >= 0 ? 
        new Quaternion(((1 - t) * storage[0]) + (t * q2.x),
                       ((1 - t) * storage[1]) + (t * q2.y), 
                       ((1 - t) * storage[2]) + (t * q2.z), 
                       ((1 - t) * storage[3]) + (t * q2.w)) :    
        new Quaternion(((1 - t) * storage[0]) - (t * q2.x), 
                       ((1 - t) * storage[1]) - (t * q2.y), 
                       ((1 - t) * storage[2]) - (t * q2.z), 
                       ((1 - t) * storage[3]) - (t * q2.w));
    copyFrom(q.scale(1 / q.length));
    return this;
  }
  
  /// Spherically interpolates between [q1] and [q2] by [t].
  static Quaternion slerp(Quaternion q1, Quaternion q2, double t) {
    ...
    q.w = (q1.w * ratioA + q.w * ratioB);
    q.x = (q1.x * ratioA + q.x * ratioB);
    q.y = (q1.y * ratioA + q.y * ratioB);
    q.z = (q1.z * ratioA + q.z * ratioB);
    q._updateEuler();
    return q;
  }
  
  /// Initialized with values from [array] starting at [offset].
  Quaternion.array(List<double> array, [int offset = 0]) : storage = new Float32List(4) {
    int i = offset;
    storage[3] = array[i+3];
    storage[2] = array[i+2];
    storage[1] = array[i+1];
  ...
  Quaternion copyFromArray(List<double> array, [int offset = 0]) {
  ...
  /// Set quaternion with rotation of [angle] around normalized [axis].
  Quaternion setFromAxisAngle(Vector3 axis, double angle) {
    var halfAngle = radians / 2, s = Math.sin(halfAngle);
    storage[0] = axis.x * s;
    storage[1] = axis.y * s;
    storage[2] = axis.z * s;
    storage[3] = Math.cos(halfAngle);
    _updateEuler();
    return this;
  }

  /// Set quaternion with rotation of [euler].
