...
class Matrix4 {
  ...
  Matrix4.rotationX(double radians) : storage = new Float32List(16) {
    storage[15] = 1.0;
    setRotationX(radians);
  }

  /// Rotation of [radians_] around Y.
  Matrix4.rotationY(double radians) : storage = new Float32List(16) {
    storage[15] = 1.0;
    setRotationY(radians);
  }

  /// Rotation of [radians] around Z.
  Matrix4.rotationZ(double radians) : storage = new Float32List(16) {
    storage[15] = 1.0;
  ...
  /// Scale matrix.
  Matrix4.diagonal3(Vector3 scale) : storage = new Float32List(16) {
    storage[15] = 1.0;
    storage[10] = scale.storage[2];
    storage[5] = scale.storage[1];
    storage[0] = scale.storage[0];
  }
  ...
  dynamic operator*(dynamic arg) {
    if (arg is double) return _mul_scale(arg);
    if (arg is Vector4) return _mul_vector(arg);
    if (arg is Vector3) return _mul_vector3(arg);
    if (4 == arg.dimension) return _mul_matrix(arg);
  ...
  /*
   *  Additions based on three.js matrix4
   */
  ...
  Matrix4.orthographic(double left, double right, double top, double bottom, double near, double far)
      : storage = new Float32List(16) {
    double w = right - left;
    double h = top - bottom;
    double p = far - near;
    ...
    storage[0] = 2 / w;  storage[4] = 0.0;    storage[8]  = 0.0;     storage[12] = -x;
    storage[1] = 0.0;    storage[5] = 2 / h;  storage[9]  = 0.0;     storage[13] = -y;
    storage[2] = 0.0;    storage[6] = 0.0;    storage[10] = -2 / p;  storage[14] = -z;
  ...
  Matrix4.perspective(double fov, double aspect, double near, double far) 
      : storage = new Float32List(16) {
    double ymax = near * Math.tan(MathUtils.degToRad(fov * 0.5));
    double ymin = -ymax;
  ...
  Matrix4 makeFrustum(double left, double right, double bottom, double top, double near, double far) {
    ...
    storage[0] = x;      storage[4] = 0.0;    storage[8]  = a;      storage[12] = 0.0;
    storage[1] = 0.0;    storage[5] = y;      storage[9]  = b;      storage[13] = 0.0;
    storage[2] = 0.0;    storage[6] = 0.0;    storage[10] = c;      storage[14] = d;
    storage[3] = 0.0;    storage[7] = 0.0;    storage[11] = -1.0;   storage[15] = 0.0;
  }
  
  /// Extracts the scale of this matrix transform.
  Vector3 getScale() {
    double sx = new Vector3(storage[0], storage[1], storage[2]).length;
    double sy = new Vector3(storage[4], storage[5], storage[6]).length;
    double sz = new Vector3(storage[8], storage[9], storage[10]).length;
    return new Vector3(sx, sy, sz);
  }
  
  ///Sets the rotation submatrix of this matrix to the rotation specified by [euler].
  Matrix4 makeRotationFromEuler(Euler euler) {
    ...
    // last column
    storage[3] = storage[7] = storage[11] = 0.0;
    // bottom row
    storage[12] = storage[13] = storage[14] = 0.0;
    storage[15] = 1.0;
    return this;
  }
  ...
  Matrix3 getNormalMatrix() {
    var m = new Matrix4.copy(this)
    ..invert()
    ..transpose();
    return m.getRotation();
  }
  
  void multiplyDoubleArray(List<double> array) {
    for (var i = 0; i < array.length; i += 3) {
      var v = new Vector3.array(array, i)..applyProjection(this);
      array[i]   = v.x;
      array[i+1] = v.y;
      array[i+2] = v.z;
    } 
  ...
  Sphere transformSphere(Sphere sphere) {
    transform3(sphere.center);
    sphere.radius = sphere.radius * getMaxScaleOnAxis();
    return sphere;
  }
  
  Ray transformRay(Ray ray) {
    transform3(ray.direction..add(ray.origin));
    transform3(ray.origin);
